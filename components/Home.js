...
useEffect(() => {
  ...
  const movies = moviesData.map((data, i) => {
    ...
    let overviewLength = data.overview.substr(0,250);
    if(data.overview.length> 250) {
      overviewLength += "..."
    }
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={overviewLength} poster={`https://image.tmdb.org/t/p/w500/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
